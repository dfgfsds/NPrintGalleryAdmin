...
export default function Login() {
  ...
  const onSubmit = async (data: any) => {
    ...
    setError('');
    try {
      const updateApi = await postLoginInAPi({...data,mainmultivendor_id:42});
      // const updateApi = await postLoginInAPi({...data,mainmultivendor_id:1});
      if (updateApi?.data) {
        localStorage.setItem('userId', updateApi?.data?.user_id);
        localStorage.setItem('mainVendor', updateApi?.data?.main_multivendor_id);
        navigate('/store/134/products');
        // navigate('/dashboard');
        // navigate('/store/1/products');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-12">
      ...
      <div className='flex justify-center '>
          <img src={Logo} className='w-full h-40'/>
        </div>
         <div>
